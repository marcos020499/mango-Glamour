  ...
  switch (type) {
    case UPDATE_EMAIL: {
      try {
        ...
        yield call(displayActionMessage, 'Email actualizado!', 'completado');
      } catch (e) {
    ...
    case UPDATE_PROFILE: {
      try {
        ...
        yield call(displayActionMessage, 'Perfil actualizado exitosamente!', 'completado');
      } catch (e) {
        console.log(e);
        yield put(setLoading(false));
        if (e.code === 'auth/wrong-password') {
          yield call(displayActionMessage, 'Contraseña incorrecta, actualizacion de perfil incorrecta :(', 'error');
        } else {
          yield call(displayActionMessage, `:( Error al actualizar perfil. ${e.message ? e.message : ''}`, 'error');
        }
      }
      break;
    }
    default: {
      throw new Error('Inesperada accion.');
    }
  }
}
